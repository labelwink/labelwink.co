...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-5 max-w-[1200px]">
            ...
            {DAY_OPTIONS.map(d => (
              <button key={d} onClick={() => setDays(d)}
                className={`px-3.5 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  days === d ? 'bg-white shadow text-[#1b3a34]' : 'text-[#6b7280] hover:text-[#1b3a34]'
                }`}>
                {d}d
              </button>
            ))}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {loading ? (
          ...
          <div key={i} className="bg-white border border-[#e5e7eb] rounded-xl p-5 hover:shadow-sm transition-shadow">
            ...
            <p className="text-xl font-bold text-[#1b3a34]">{kpi.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white border border-[#e5e7eb] rounded-xl p-5">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-sm font-semibold text-[#1b3a34] flex items-center gap-2">
              ...
              {CHART_MODE_OPTIONS.map(m => (
                <button key={m} onClick={() => setChartMode(m)}
                  className={`px-2.5 py-1 rounded text-[10px] font-semibold capitalize transition-colors ${
                    chartMode === m ? 'bg-white shadow text-[#1b3a34]' : 'text-[#6b7280]'
                  }`}>
                  {m}
                </button>
              ))}
        ...
        <div className="bg-white border border-[#e5e7eb] rounded-xl p-5">
          <h3 className="text-sm font-semibold text-[#1b3a34] mb-4">Order Status</h3>
          {loading ? (
                ...
                return (
                  <div key={status}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[10px] font-bold text-[#1b3a34]">{count}</span>
                    </div>
      ...
      {/* Top Products */}
      <div className="bg-white border border-[#e5e7eb] rounded-xl p-5">
        <h3 className="text-sm font-semibold text-[#1b3a34] mb-4 flex items-center gap-2">
              ...
              return (
                <div key={i} className="flex items-center gap-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-[#1b3a34] truncate">{p.product_name}</p>
                  ...
                  <span className="text-xs font-bold text-[#1b3a34] w-20 text-right flex-shrink-0">
